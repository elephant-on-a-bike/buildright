(function(){
  ...
  function createField(field) {
    ...
    label.htmlFor = 'contractor_' + field.id;
  ...
  function createTextarea(field) {
    ...
    textarea.id = 'contractor_' + field.id;
  ...
  function createSelect(field) {
    ...
    select.id = 'contractor_' + field.id;
  ...
  function createRadio(field) {
    ...
    field.options.forEach((option, index) => {
      ...
      const input = document.createElement('input');
      input.type = 'radio';
      input.id = `contractor_${field.id}_${index}`;
      input.name = field.id;
      input.value = option;
      if (field.required) input.required = true;
  ...
  function createFileInput(field) {
    ...
    preview.id = `contractor_${field.id}_preview`;
  ...
  contractorForm?.addEventListener('submit', async (e) => {
    ...
    const professionalRecord = {
      ...
      type: 'contractor', // contractor, reseller
      businessType: 'sole_trader', // sole_trader or company (can be determined from form data)
      registrationDate: new Date().toISOString(),
      status: 'pending', // pending, approved, rejected
      data: contractorData
    };

    console.log('Contractor registration:', professionalRecord);
    ...
    try {
      ...
      const formContainer = contractorForm.closest('.join-form-wrapper');
      if (formContainer) {
        ...
        if (leftPanel) {
          leftPanel.innerHTML = `
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:3rem 2rem;text-align:center;min-height:400px;">
              ...
              <p style="color:#6B7280;margin:0 0 1.5rem;">Your contractor registration has been received. You will be notified once reviewed.</p>
